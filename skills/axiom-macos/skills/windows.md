...
- The reference-type document model — `@Observable` document classes via `ReadableDocument`/`WritableDocument` and the matching `DocumentGroup` initializers (`OS27`) — is not covered here; the scene-level wiring here is unchanged.
