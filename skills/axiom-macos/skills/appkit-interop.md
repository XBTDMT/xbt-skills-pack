...
Observation-tracking methods: anything called as part of `NSView.draw(_:)` (incl. `NSCell` draw methods like `drawKnob`/`drawBar`), plus `updateConstraints()`, `layout()`, `updateLayer()`, and the `NSViewController` equivalents. UIKit's list is larger (extends to `UIButton`, `UICollectionViewCell`, …).
...
The protocol is macOS-only at macOS 26; the UIKit counterpart `UIGestureRecognizerRepresentable` is not covered here.
...
**Skills**: skills/appkit-modernization.md, axiom-swiftui
