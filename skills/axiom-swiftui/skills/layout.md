...
(Switching layout on `dynamicTypeSize` is layout. Verifying the result at accessibility sizes is an accessibility audit, outside this skill.)
...
UIKit's `readableContentGuide` does all of this automatically, including the Dynamic Type response. Apply the cap to *text columns*, not to grids or media, which should keep using the full width.
...
**Skills**: skills/layout-ref.md, skills/debugging.md
