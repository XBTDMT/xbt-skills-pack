...
**iOS 26 note** Liquid Glass changes how toolbar backgrounds render. Review Apple's Liquid Glass guidance before customizing background materials in iOS 26+ apps.
...
Navigation and bottom bars can minimize as the user scrolls — the SwiftUI counterpart of UIKit's `UINavigationItem.navigationBarMinimization` (`UIBarMinimization`). Three modifiers, each scoped by `ToolbarPlacement`:
...
UIKit peers (`UINavigationItem.subtitle`, `largeSubtitleView`, and their appearance gotchas) are UIKit territory, not covered here.
...
- [ ] iOS 26+ apps reviewed against Apple's Liquid Glass guidance for background-material changes
...
**Skills**: axiom-swiftui (skills/nav.md), axiom-macos (skills/windows.md), axiom-macos (skills/menus-and-commands.md)
