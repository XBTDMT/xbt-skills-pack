...
**Spacing values are not shown here on purpose.** Every stack and grid takes `spacing:` as a `CGFloat?`, and omitting it yields system spacing — the correct default. Which container to use is a layout question (this file); how much space to put between things is a design question, answered by the Human Interface Guidelines. The one example below that passes an explicit value (the photo grid) says why.
