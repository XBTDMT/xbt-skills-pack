...
- **On 27, profile the peak, not the resting figure.** Eviction means rows are rebuilt on the way back, and the rebuild spike outruns sampling — a list measured on device peaked at several times its resting footprint while per-frame polling saw almost nothing. Numbers in `skills/layout-ref.md`.
...
**Swift 6.4 fast path**: when the invalidation check itself compares a large CoW collection with `==` on every update, `isTriviallyIdentical(to:)` (SE-0494) can answer "unchanged?" in O(1) — but `false` means "no information", not "changed", and fresh-built inputs make it a pessimization. Adoption conditions and the trap are in the SE-0494 proposal.
...
**Skills**: skills/debugging-diag.md, skills/debugging.md
